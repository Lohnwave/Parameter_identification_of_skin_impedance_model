...
%function[Error,X_gbest,gbestval,allgbestval,fitcount] = EBS_SCA_PV(fhd,max_iteration,max_FES,N,D,lb,ub,varargin) 
function[gbestval,allgbestval,X_gbest,fitcount] = EBS_SCA_ZK(max_iteration,max_FES,N,D,lb,ub) 
%function [gbestval,allgbestval,gbest,fitcount]= ABC_SCA01_PV(max_iteration,max_FES,Particle_Number,Dimension,VRmin,VRmax,fobj,varargin)
		%Load Target value
        %load fbias_data;
        % *************** Initialization *******************
        M = 10;
        ...
        while n_iteration<=max_iteration && fitcount<=max_FES                
            ...
            if mod(n_iteration,100)==0
                disp(['iter: ',num2str(n_iteration),'   gebestVal: ',num2str(gbestval)]);
            end
